...
recipe_digest="$(cd "$recipe_dir" && shasum -a 256 Dockerfile inputs.env apk.sha256 build.sh validate.py lock.py seccomp-exec.patch tenant-root.patch | shasum -a 256 | cut -d' ' -f1)"
