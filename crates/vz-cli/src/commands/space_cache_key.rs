...
mod tests {
    ...
    fn schema_version_mismatch_is_explicit_and_invalidation_is_deterministic() {
        let mut index = SpaceCacheIndex::default();
        let key_v1 = SpaceCacheKey::from_material_with_version(fixture_material(), 1).expect("v1");
        index.upsert(key_v1);

        let key_v2 = SpaceCacheKey::from_material_with_version(fixture_material(), 2).expect("v2");
        assert_eq!(
            index.lookup(&key_v2),
        ...
        let invalidated = index.invalidate_for_schema(2);
        assert_eq!(invalidated, 1);
        assert_eq!(index.lookup(&key_v2), SpaceCacheLookup::MissNotFound);
    }
}
