...
async fn main() -> anyhow::Result<()> {
    ...
    let operation = async {
        ...
        let record = serde_json::json!({
            ...
            "machine_directory":machine,"loader":pong,"guest_probe":probe,
            "agent_os_version":info.os_version,"agent_protocol_revision":info.agent_protocol_revision,
            "boot_to_agent_seconds":started.elapsed().as_secs_f64(),"fresh_machine_identifier":true,
            "consumer_e2e_validated":false
        });
        std::fs::write(
            machine.join("probe.json"),
    ...
    if *state.borrow() != VmState::Stopped {
        vm.stop().await.context("stop owned probe VM")?;
    }
    result
}
