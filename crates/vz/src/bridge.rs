...
pub(crate) fn build_objc_config(
    ...
    match &config.network {
        NetworkConfig::Nat => {
            ...
            let mac_string = NSString::from_str(&config.network_mac);
            // SAFETY: initWithString validates the MAC string and returns nil on a
            // malformed input. The string is generated by Apple's API or supplied by
            // the consumer; we surface the parse failure as a config error.
            let mac = unsafe {
