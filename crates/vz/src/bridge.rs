...
use objc2_virtualization::{
    ...
    VZLinuxBootLoader, VZMacAuxiliaryStorage, VZMacGraphicsDeviceConfiguration,
...
pub(crate) fn nsurl_from_path(path: &Path) -> Retained<NSURL> {
    ...
    // SAFETY: initFileURLWithPath is safe starting from objc2-foundation 0.3.2,
    // but marked unsafe in 0.3.1. Allow unused_unsafe for cross-version compat.
...
/// Generate a fresh, random, unicast, locally-administered MAC address.
///
/// Returns the canonical `"xx:xx:xx:xx:xx:xx"` form, ready to feed back into
/// `VZMACAddress::initWithString:` at VM-construction time. Used by
/// `VmConfigBuilder::build()` when the caller did not pin a MAC explicitly.
/// Parse a `"XX:XX:XX:XX:XX:XX"` MAC into the framework's type.
///
/// Every NIC pins its MAC. Without it `VZVirtioNetworkDeviceConfiguration::new()`
/// randomizes the address on every construction, so a restored VM's NIC would not
/// match the one the saved guest expects, and two VMs in one process would collide.
fn parse_mac(mac: &str) -> Result<Retained<objc2_virtualization::VZMACAddress>, VzError> {
    let text = NSString::from_str(mac);
    // SAFETY: initWithString validates the MAC string and returns nil on a
    // malformed input. The string is generated by Apple's API or supplied by the
    // consumer; we surface the parse failure as a config error.
    unsafe {
    ...
    .ok_or_else(|| VzError::InvalidConfig(format!("invalid MAC address format: {mac:?}")))
}

pub(crate) fn random_locally_administered_mac_string() -> String {
    // SAFETY: randomLocallyAdministeredAddress is a class method on VZMACAddress
    // that always returns a valid retained instance.
    let mac = unsafe { objc2_virtualization::VZMACAddress::randomLocallyAdministeredAddress() };
    // SAFETY: -[VZMACAddress string] returns a retained NSString of the address.
    let s = unsafe { mac.string() };
...
pub(crate) fn build_objc_config(
    ...
    // Storage devices (one virtio-block per DiskConfig, in declaration order).
    if !config.disks.is_empty() {
        let mut block_devices = Vec::with_capacity(config.disks.len());
        for disk in &config.disks {
            let disk_url = nsurl_from_path(&disk.path);
            ...
            .map_err(|e| {
                VzError::DiskError(format!("disk {:?}: {}", disk.id, ns_error_to_string(&e)))
            })?;
            ...
            block_devices.push(Retained::into_super(block_device));
        }
        // SAFETY: NSArray::from_retained_slice creates an array from a slice of retained objects.
        let storage_devices = NSArray::from_retained_slice(&block_devices);
    ...
    // and the guest agent never starts. When no log file is specified, write to /dev/null.
    {
        let file_handle = if let Some(serial_log_file) = &config.serial_log_file {
            ...
            // SAFETY: safe in objc2-foundation 0.3.2+, unsafe in 0.3.1.
            #[allow(unused_unsafe)]
