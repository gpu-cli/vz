...
pub(super) async fn export_space_cache(
    State(state): State<ApiState>,
    headers: HeaderMap,
    raw_body: axum::body::Bytes,
) -> Response {
    let request_id = request_id_from_headers(&headers);
    if let Some(response) =
        try_export_space_cache_via_daemon(&state, raw_body.as_ref(), &request_id).await
    {
        return response;
    }
    json_error_response(
...
pub(super) async fn import_space_cache(
    State(state): State<ApiState>,
    headers: HeaderMap,
    raw_body: axum::body::Bytes,
) -> Response {
    let request_id = request_id_from_headers(&headers);
    if let Some(response) =
        try_import_space_cache_via_daemon(&state, &headers, raw_body.as_ref(), &request_id).await
    {
        return response;
    }
    json_error_response(
