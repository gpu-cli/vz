...
use super::space_cache::SpaceCacheKey;
...
mod tests {
    ...
    fn write_signed_fixture() -> SignedFixture {
        ...
        let keypair = Ed25519KeyPair::from_pkcs8(pkcs8.as_ref()).expect("decode keypair");
        let signature = keypair.sign(&manifest_bytes);
        let signature_path = artifact.join("signature.sig");
        std::fs::write(&signature_path, signature.as_ref()).expect("write signature");

        let blob_path = artifact.join("payload.tar.zst");
