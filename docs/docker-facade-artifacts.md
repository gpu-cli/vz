# Linux Developer Environment Docker artifacts and lazy supervision

Status: **DEV** — shared artifact provisioning and supervision seam implemented;
per-environment state migration, endpoint integration, and Linux VM E2E pending.

Docker compatibility is an implicit capability of each Linux Developer
Environment. Reconciliation provisions or verifies its requirements; the
environment's private Engine may still start lazily on first API traffic. The
host endpoint adapter (`vz-7ez`) calls the guest agent's server-streamed
`EnsureDocker` RPC for the selected environment. Native macOS and native
Windows targets do not use this contract.

## Host artifact and environment-state layout

`ensure_docker_artifacts()` owns the checksum-pinned, immutable binary cache.
Mutable state must be owned by a stable environment identity rather than the
...
<daemon-owned-environment-root>/<environment-id>/docker/
└── data.img
```

The physical environment-root and endpoint paths are backend-owned details.
Users address the environment by name/ID or its managed Docker context and do
not construct socket paths.
...
Each environment's `data.img` is a private sparse persistent disk. Docker state
is never placed in the initramfs or shared with another environment. The Linux
environment VM must attach its owned image as `/dev/vda`, expose
...
environment. On its first call, the guest agent mounts the two read-only binary
shares and requires that environment's existing ext4 `/dev/vda` mount at
...
selected environment's artifact paths and invoke `EnsureDocker` from its
private endpoint activation. Existing single-image state must migrate to
environment-owned storage. A newly created sparse `data.img` is not
