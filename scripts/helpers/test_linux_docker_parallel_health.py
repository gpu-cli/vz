...
class LifecycleTests(unittest.TestCase):
    ...
    def test_capture_failure_never_produces_success_or_leaves_thread(self):
        for result in ('exception', 'uncertain', 'nonzero'):
            ...
            with self.subTest(result=result):
                if result == 'exception':
                    # An observer that raised is caught while start() is still
                    # waiting for its first sample, so the suite never runs a
                    # workload against a probe that has already died.
                    with self.assertRaisesRegex(ValueError, 'before its first sample'):
                        selected.start()
                else:
                    # These two produce a completed observer whose record is bad;
                    # only the full record shows that, so finish() is where they
                    # are caught.
                    selected.start()
                with self.assertRaises(ValueError):
    ...
    def test_start_waits_for_the_probes_first_sample_before_returning(self):
        # The validator rejects any workload envelope that opens before the
        # probe's first sample has finished, and a composed run once lost that by
        # 21 ms. start() must not return while a running probe could still be
        # inside its first sample.
        ...
        def block(*args, **kwargs):
            running.wait(5)
            return original(*args, **kwargs)
        selected.record.run = Mock(side_effect=block)
        with patch.object(health, 'PROCESS_START_ALLOWANCE_SECONDS', 0.4):
            began = time.monotonic()
            selected.start()
            waited = time.monotonic() - began
        expected = 0.4 + (health.TIMING['max_lateness_ns'] + health.TIMING['request_timeout_ns']) / 10 ** 9
        self.assertGreaterEqual(
            waited, expected * 0.9,
            'start() returned before the probe could have finished its first sample')
        running.set()
        selected.thread.join(5)
...
class GuestProbeTests(unittest.TestCase):
    def test_actual_probe_algorithm_sixty_loopback_requests_without_retry(self):
        # The fixture tree is hash-bound and rejects generated bytecode, so
        # executing its probe here must not leave a __pycache__ behind.
