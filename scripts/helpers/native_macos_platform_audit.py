...
def main():
    ...
    evidence = parser.parse_args().evidence.resolve()
    ...
    assert len({row["identifier_sha256"] for row in machines.values()}) == 2
    for key in machines:
    ...
    for name in ["disk.img", "auxiliary-storage", "machine-identifier"]:
        assert len({row["files"][name]["inode"] for row in machines.values()}) == 2
    ...
    (evidence / "platform-audit.json").write_text(json.dumps(dict(
        ...
        scope="private platform identities across installed Stop/Up and two Environments"), indent=2))
