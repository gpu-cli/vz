...
# The host binaries this installer owns, named once. Install and uninstall both
# read this list: two hand-maintained copies drifted the moment a sixth binary
# was added on one side, which left `vz-runtime-probe` behind on uninstall and
# broke the "removes only, and all of, vz-owned software" claim.
VZ_BINARIES="vz vz-runtimed vz-guest-agent vz-agent-loader vz-macos-setup vz-runtime-probe"
