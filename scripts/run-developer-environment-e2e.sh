#!/bin/bash
# EXPLICIT FAILING STUB for the `topology` lane of the vz 0.4 aggregate gate.
...
exec uv run --no-project --python /usr/bin/python3 \
  ...
  python -B "$script_dir/helpers/vz04_lanes.py" stub --lane topology --entry-point "scripts/run-developer-environment-e2e.sh" "$@"
